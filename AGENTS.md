...
| `Source/WebCore/{ShotPruning.cmake,dom/EventInterfaces.in}` | Shot 物理移除 WebGPU IDL/生成绑定，`GPUUncapturedErrorEvent` 同步增加 `WEBGPU` 事件工厂条件 | WebGPU IDL 只有运行时 setting、没有编译期条件；实现已裁掉时残留 `JSGPU*` 绑定与 Event 工厂仍引用其 TZone allocator，导致 dylib 留下未定义符号 | M4/macOS 🚧 已改，CI 验证中 |
