...
| `Source/WebCore/html/canvas/GPUCanvasContextCocoa.mm` | OffscreenCanvas fallback 受 `ENABLE(OFFSCREEN_CANVAS)` 门控，关闭时标记不可达 | Cocoa 上游默认同时开启 OffscreenCanvas；Shot 保留 WebGPU DOM 闭包但关闭 OffscreenCanvas，原无条件 downcast 无法编译 | M4/macOS 🚧 已改，CI 验证中 |
