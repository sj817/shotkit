...
| `Source/WTF/wtf/PlatformUse.h`、`Source/WebCore/{page/Page.cpp,accessibility/AXCoreObject.h,page/ChromeClient.h,loader/EmptyClients.h,platform/PlatformKeyboardEvent.cpp}` | 增加 `PLATFORM(SHOT)` 分支；Shot 禁用 Adwaita 异步滚动条和仅 GTK/WPE 存在的硬件加速设置，复用无障碍空 wrapper 与通用 modifier 状态 | Linux Shot 是无 UI、同步滚动、软件光栅端口；避免引入 GTK/WPE 或异步滚动树 | M3/Linux ✅ 已改 |
