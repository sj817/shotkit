...
| `Source/WebCore/{ShotPruning.cmake,dom/EventInterfaces.in,page/Navigator.idl,page/WorkerNavigator.idl}` | Shot 物理移除 WebGPU IDL/生成绑定，事件工厂和 Navigator mixin 关系同步增加 WebGPU 编译条件 | WebGPU IDL 只有运行时 setting、没有编译期条件；实现已裁掉时残留 `JSGPU*` 绑定与 Event 工厂仍引用其 TZone allocator，无条件 Navigator include 还会使绑定预处理失败 | M4/macOS 🚧 已改，CI 验证中 |
