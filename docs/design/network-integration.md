...
> **状态：历史文档。** 这是 2026-07 写 M2 网络栈之前的实现规格，签名逐字摘录自
> 当时的上游源码。M2 已完工，实际实现在 `shot/kernel/ShotLoaderStrategy.{h,cpp}`
> 与 `ShotCurlResourceLoader.{h,cpp}`——以代码为准。
>
> 保留它的理由：排查网络层回归时，这份「为什么这样接」的推导比读代码快得多。
> 文中的行号锚点对应当时的上游快照，可能已漂移，以符号名为准。
