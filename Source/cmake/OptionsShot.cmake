...
if (APPLE)
    ...
    SET_AND_EXPOSE_TO_BUILD(ENABLE_IMAGE_ANALYSIS OFF)
    SET_AND_EXPOSE_TO_BUILD(ENABLE_IMAGE_ANALYSIS_ENHANCEMENTS OFF)
    SET_AND_EXPOSE_TO_BUILD(ENABLE_IMAGE_ANALYSIS_FOR_MACHINE_READABLE_CODES OFF)
    SET_AND_EXPOSE_TO_BUILD(ENABLE_MODEL_ELEMENT_ACCESSIBILITY OFF)
    SET_AND_EXPOSE_TO_BUILD(ENABLE_NOTIFICATION_EVENT OFF)
    SET_AND_EXPOSE_TO_BUILD(HAVE_AVASSETREADER OFF)
    SET_AND_EXPOSE_TO_BUILD(HAVE_AVPLAYER_VIDEORANGEOVERRIDE OFF)
    SET_AND_EXPOSE_TO_BUILD(HAVE_WEBGPU_IMPLEMENTATION OFF)
    set(ENABLE_WEBKIT OFF)
    set(ENABLE_WEBKIT_LEGACY OFF)
...
# Opt-in ThinLTO experiment: keep the default full-LTO profile unchanged.
# Bound backend parallelism on small runners and persist native objects.
...
if (LTO_MODE STREQUAL "thin")
    ...
    elseif (APPLE)
        if (SHOT_THINLTO_CACHE_DIR)
            string(APPEND _shot_thin_lto_link_flags " \"-Wl,-cache_path_lto,${SHOT_THINLTO_CACHE_DIR}\"")
        endif ()
    else ()
        # The Linux Shot toolchain uses ELF lld.
        if (SHOT_LTO_JOBS)
            string(APPEND _shot_thin_lto_link_flags " -Wl,--thinlto-jobs=${SHOT_LTO_JOBS}")
        endif ()
        if (SHOT_THINLTO_CACHE_DIR)
            string(APPEND _shot_thin_lto_link_flags " \"-Wl,--thinlto-cache-dir=${SHOT_THINLTO_CACHE_DIR}\"")
        endif ()
    endif ()
